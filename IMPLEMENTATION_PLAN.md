...
Public submission creation should go through a same-origin server endpoint, not direct anonymous table inserts. The endpoint may use a server-only service-role client after validation and rate limiting. Never expose `SUPABASE_SERVICE_ROLE_KEY` or any privileged credential in `NEXT_PUBLIC_*`, Vite `VITE_*`, browser code, logs, or error payloads.
