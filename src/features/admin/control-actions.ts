...
export async function saveCampaignAction(formData: FormData) {
  ...
  const fallback =
    parsed.success && parsed.data.id ? `/admin/kampanyok/${parsed.data.id}` : '/admin/kampanyok';
  ...
  if (error || !id.success)
    redirectWith(
      fallback,
      'error',
      error?.code === '23505' ? 'Ez a slug már használatban van.' : 'A kampány nem menthető.',
    );
  revalidatePath('/admin');
  revalidatePath('/admin/kampanyok');
  revalidatePath(`/admin/kampanyok/${id.data}`);
...
export async function setCampaignActiveAction(formData: FormData) {
  ...
  if (error)
    redirectWith(
      ...
      'error',
      error.code === '23505'
        ? 'Már van aktív kampány. Előbb deaktiváld azt.'
        : 'A kampány állapota nem módosítható.',
    );
  ...
  redirectWith(
    path,
    'success',
    active.data === 'true' ? 'A kampány aktív.' : 'A kampány deaktiválva lett.',
  );
}
...
export async function setCampaignSchoolAction(formData: FormData) {
  ...
  const client = await mutationClient('admin');
  if (!client || !campaignId.success || !schoolId.success || !active.success)
    redirectWith(path, 'error', 'Érvénytelen kérés.');
  const { error } = await client.rpc('admin_set_campaign_school', {
    requested_campaign_id: campaignId.data,
    requested_school_id: schoolId.data,
    requested_active: active.data === 'true',
  });
  ...
  redirectWith(
    path,
    'success',
    active.data === 'true' ? 'Az iskola hozzáadva.' : 'Az iskola részvétele deaktiválva.',
  );
}
...
export async function bulkCampaignSchoolsAction(formData: FormData) {
  const schema = z.object({
    ...
    participation: z.enum(['all', 'selected', 'unselected']),
  ...
  const parsed = schema.safeParse({
    ...
    participation: formData.get('participation') ?? 'all',
  ...
  const { data, error } = await client.rpc('admin_bulk_set_filtered_campaign_schools', {
    ...
    requested_county: parsed.data.county,
    requested_city: parsed.data.city,
    requested_participation: parsed.data.participation,
...
export async function createSchoolAction(formData: FormData) {
  const parsed = schoolSchema.omit({ id: true, active: true }).safeParse({
    name: formData.get('name'),
    slug: formData.get('slug'),
  ...
  if (!client || !parsed.success) {
    redirectWith('/admin/iskolak/uj', 'error', 'Ellenőrizd az iskola adatait.');
  }
  const { data, error } = await client.rpc('admin_create_school', {
    ...
    requested_city: parsed.data.city,
    requested_county: parsed.data.county,
    requested_postal_code: parsed.data.postalCode || null,
    requested_address: parsed.data.address || null,
  });
  const id = z.uuid().safeParse(data);
  if (error || !id.success) {
    redirectWith(
      '/admin/iskolak/uj',
      'error',
      error?.code === '23505' ? 'Ez a slug már használatban van.' : 'Az iskola nem hozható létre.',
    );
  }
  revalidatePath('/admin/iskolak');
  revalidatePath('/admin');
  redirectWith(`/admin/iskolak/${id.data}`, 'success', 'Az iskola létrejött.');
}
...
export async function updateSchoolAction(formData: FormData) {
  ...
  const { error } = await client.rpc('admin_update_school', {
    requested_school_id: parsed.data.id,
    requested_name: parsed.data.name,
    requested_slug: parsed.data.slug,
    requested_type: parsed.data.type,
    requested_city: parsed.data.city,
  ...
  if (error)
    redirectWith(
      path,
      'error',
      error.code === '23505' ? 'Ez a slug már használatban van.' : 'Az iskola nem menthető.',
    );
  revalidateTag('public-campaign', 'max');
  revalidatePath('/admin/iskolak');
  revalidatePath(path);
...
export async function setSchoolActiveAction(formData: FormData) {
  const id = z.uuid().safeParse(formData.get('school_id'));
  const active = z.enum(['true', 'false']).safeParse(formData.get('active'));
  const path = id.success ? `/admin/iskolak/${id.data}` : '/admin/iskolak';
  const client = await mutationClient('admin');
  if (!client || !id.success || !active.success) {
    redirectWith(path, 'error', 'Érvénytelen kérés.');
  }
  const { error } = await client.rpc('admin_set_school_active', {
    requested_school_id: id.data,
    requested_active: active.data === 'true',
  });
  if (error) redirectWith(path, 'error', 'Az iskola állapota nem módosítható.');
  revalidateTag('public-campaign', 'max');
  revalidatePath('/admin/iskolak');
  revalidatePath(path);
  revalidatePath('/admin');
  ...
  redirectWith(
    path,
    'success',
    active.data === 'true' ? 'Az iskola ismét aktív.' : 'Az iskola deaktiválva lett.',
  );
}
...
export async function saveNewsAction(formData: FormData) {
  const parsed = newsSchema.safeParse({
    id: formData.get('news_id'),
    title: formData.get('title'),
    slug: formData.get('slug'),
    excerpt: formData.get('excerpt'),
    content: formData.get('content'),
    published: formData.get('published') === 'on',
    publishedAt: formData.get('published_at') || null,
  });
  const fallback =
  ...
  const { data, error } = await client.rpc('admin_save_news', {
    requested_news_id: parsed.data.id,
    requested_title: parsed.data.title,
    requested_slug: parsed.data.slug,
    requested_excerpt: parsed.data.excerpt,
    requested_content: parsed.data.content,
    requested_published: parsed.data.published,
    requested_published_at: publishedAt?.toISOString() ?? null,
  });
  const id = z.uuid().safeParse(data);
  if (error || !id.success)
    redirectWith(
...
async function findAuthUserByEmail(
  privileged: NonNullable<ReturnType<typeof createPrivilegedSupabaseClient>>,
  email: string,
) {
...
export async function inviteAdministratorAction(formData: FormData) {
  ...
  if (!client || !parsed.success)
    redirectWith('/admin/adminisztratorok', 'error', 'Ellenőrizd a meghívási adatokat.');
  ...
  const existingUser = await findAuthUserByEmail(privileged, parsed.data.email);
  const invitation = existingUser
    ? null
    : await privileged.auth.admin.inviteUserByEmail(parsed.data.email, {
  ...
  if (error) {
    if (!existingUser) await privileged.auth.admin.deleteUser(user.id);
    redirectWith('/admin/adminisztratorok', 'error', 'A meghívott jogosultsága nem menthető.');
  }
  revalidatePath('/admin/adminisztratorok');
  redirectWith(
    '/admin/adminisztratorok',
    'success',
    existingUser ? 'A meglévő fiók jogosultsága frissült.' : 'A meghívó elküldve.',
  );
}
...
export async function manageAdministratorAction(formData: FormData) {
  ...
  const client = await mutationClient('super_admin');
  if (!client || !parsed.success)
    redirectWith('/admin/adminisztratorok', 'error', 'Érvénytelen kérés.');
  const { error } = await client.rpc('admin_manage_administrator', {
  ...
  if (error)
    redirectWith(
      ...
      'error',
      error.code === '55000'
        ? 'Az utolsó aktív Super Admin nem módosítható így.'
        : 'A jogosultság nem módosítható.',
    );
  revalidatePath('/admin/adminisztratorok');
  revalidatePath('/admin');
