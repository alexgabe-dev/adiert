...
export function authErrorMessage(error: AuthFailure): string {
  if (error.code === 'over_email_send_rate_limit')
    return 'Most nem tudunk megerősítő e-mailt küldeni, mert elértük a levélküldési keretet. Próbáld meg később. Ha a hiba megmarad, jelezd nekünk: info@adiert.hu.';
  ...
  if (error.code === 'email_address_not_authorized' || error.code === 'email_provider_disabled')
    return 'Az e-mail-küldés jelenleg nincs megfelelően beállítva. Kérjük, jelezd nekünk: info@adiert.hu.';
