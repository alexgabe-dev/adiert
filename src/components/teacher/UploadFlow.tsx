...
export function UploadFlow({ school, revision }: { school: string; revision?: TeacherSubmission }) {
  ...
  function next() {
    ...
    if (step === 2 && !uploadDetailsSchema.safeParse({ count, date, note }).success) {
      setError(messages.invalid_details!);
      return;
    }
    setStep(step + 1);
  }
  ...
  if (step === 4)
    return (
      <div className="rounded-3xl bg-white p-8 text-center">
        ...
        <h1 className="mt-6 text-3xl font-extrabold">Ez szép munka volt!</h1>
  ...
  return (
    <div className="mx-auto max-w-xl">
      <h1 className="text-3xl font-extrabold">
        {revision ? 'Tegyük rendbe együtt.' : 'Mutasd a mai gyűjtést!'}
      </h1>
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-5 sm:p-7">
        {step === 1 && (
          <>
            <h2 className="text-xl font-bold">Egy jól olvasható képernyőfotó.</h2>
            ...
            <div className="grid grid-cols-2 gap-3">
              <label className="flex min-h-24 cursor-pointer flex-col items-center justify-center gap-2 rounded-2xl bg-blue-50 p-3 text-sm font-bold text-blue-700">
              ...
              <label className="flex min-h-24 cursor-pointer flex-col items-center justify-center gap-2 rounded-2xl border border-slate-200 p-3 text-sm font-bold">
          ...
          <div className="space-y-5">
            <label className="block text-sm font-bold">
              ...
              <input
                ...
                className="mt-3 w-full rounded-2xl border border-blue-200 bg-blue-50 p-5 text-center text-4xl font-extrabold outline-blue-500"
              />
            </label>
            <p className="text-center text-sm text-blue-700">
              {(Number(count || 0) * 50).toLocaleString('hu-HU')} Ft felajánlás
            </p>
            <label className="block text-sm font-bold">
              Visszaváltás dátuma
              <input
            ...
            <label className="block text-sm font-bold">
              Megjegyzés {Number(count) < 50 ? '(50 palack alatt kötelező)' : '(nem kötelező)'}
              <textarea
                value={note}
                maxLength={500}
                onChange={(e) => {
        ...
        {step === 3 && (
          <div>
            <h2 className="text-xl font-bold">Minden rendben?</h2>
        ...
        <div className="mt-6 flex gap-3">
          ...
          <button
            ...
            className="flex min-h-12 flex-1 items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 py-4 font-bold text-white transition active:scale-[.98] disabled:opacity-60"
          >
            {pending ? (
              <>
